...
class DDIM(DDPM):
    ...
    def sample_backward(
        self, img_or_shape, net, device, simple_var=True, ddim_step=20, eta=1
    ):
        if simple_var:
            eta = 1
        ...
        for i in tqdm(
            ...
            t_tensor = (
                torch.tensor([cur_t] * batch_size, dtype=torch.long)
                .to(device)
                .unsqueeze(1)
            )
            eps = net(x, t_tensor)
            ...
            second_term = (
                (1 - ab_prev - var) ** 0.5 - (ab_prev * (1 - ab_cur) / ab_cur) ** 0.5
            ) * eps
            if simple_var:
